...
function keypoints = findExtrema(I_dog, I_blur, initSigma, intSize, zoomLevel)
		...
		for j = 3:M-2
			for k = 3:N-2
				...
				if ( (isMaxima(val, nb) || isMinima(val, nb)) && checkEdgeResponse(dog, j, k) )
					...
					% discard if values too large
					if (abs(keyval(1)) > 2 || abs(keyval(2)) > 2 ... 
						|| abs(keyval(3)) > 2)
						continue;
					end
